...
describe('KinBase#sign', function() {
  ...
  it("can sign an string properly", function() {
    ...
    let actualSig   = KinBase.sign(data, secretKey).toString('hex');
...
describe('KinBase#verify', function() {
  ...
  it("can verify an string properly", function() {
    let data = "hello world";
    expect(KinBase.verify(data, sig, publicKey)).to.be.truthy;
    expect(KinBase.verify("corrupted", sig, publicKey)).to.be.falsey;
    expect(KinBase.verify(data, badSig, publicKey)).to.be.falsey;
  });
